...
class UseBundleGetParcelableArrayListDetector : Detector(), SourceCodeScanner {
    companion object {
        val ISSUE = Issue.create(
            id = "UseBundleGetParcelableArrayList",
            briefDescription = "Use newer getParcelableArrayList or deprecated getParcelableArrayList",
            explanation = "Use `getParcelableArrayListOrNull` or getParcelableArrayListOrThrow",
            category = Category.PRODUCTIVITY,
            priority = 5,
            severity = Severity.WARNING,
            androidSpecific = true,
            implementation = Implementation(
                UseBundleGetParcelableArrayListDetector::class.java,
    ...
    override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
        ...
        context.report(
            Incident(
                ...
                message = "Use getParcelableArrayListOrNull or getParcelableArrayListOrThrow",
                fix = quickFix(node)
            )
        )
    }
    ...
    private fun quickFix(node: UCallExpression): LintFix {
        return fix()
            .group()
            .add(node.toLintFix("getParcelableArrayList", "getParcelableArrayListOrThrow"))
            .add(node.toLintFix("getParcelableArrayList", "getParcelableArrayListOrNull"))
            .build()
    }
