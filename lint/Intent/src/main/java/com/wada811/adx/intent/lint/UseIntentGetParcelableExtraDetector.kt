...
class UseIntentGetParcelableExtraDetector : Detector(), SourceCodeScanner {
    companion object {
        val ISSUE = Issue.create(
            id = "UseIntentGetParcelableExtra",
            briefDescription = "Use newer getParcelableExtra or deprecated getParcelableExtra",
            explanation = "Use `getParcelableExtraOrNull` or requireParcelableExtra",
            category = Category.CORRECTNESS,
            priority = 5,
            severity = Severity.WARNING,
            androidSpecific = true,
            implementation = Implementation(
                UseIntentGetParcelableExtraDetector::class.java,
    ...
    override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
        ...
        context.report(
            Incident(
                ...
                message = "Use getParcelableExtraOrNull or requireParcelableExtra",
                fix = quickFix(node)
            )
        )
    }
    ...
    private fun quickFix(node: UCallExpression): LintFix {
        return fix()
            .group()
            .add(node.toLintFix("getParcelableExtra", "requireParcelableExtra"))
            .add(node.toLintFix("getParcelableExtra", "getParcelableExtraOrNull"))
            .build()
    }
